...
if (!class_exists('WPGraphQLGutenberg')) {
    final class WPGraphQLGutenberg
    {
        ...
        public function get_graphql_block_interface_type()
        {
            if (!isset($this->graphql_block_interface_type)) {
                ...
                register_graphql_interface_type(
                    $type_name,
                    $this->get_graphql_block_interface_type_config($type_name)
                );
         ...
         * Store meta data
         */
        private function storePostMetaData($value, $post, $metaFieldName) {
            ...
            add_action('rest_api_init', function () {
                ...
                foreach (array_merge($editor_post_types, ['wp_block'])
                    ...
                    register_rest_field(
                        ...
                        array(
                            'update_callback' => function ($value, $post) {
                                $this->storePostMetaData($value, $post, WPGraphQLGutenberg::$field_name_preview);
                            },
                    ...
                    register_rest_field(
                        ...
                        array(
                            'update_callback' => function ($value, $post) {
                                $this->storePostMetaData($value, $post, WPGraphQLGutenberg::$field_name);
                            },
                            'permission_callback' => function () {
                                return current_user_can('edit_others_posts');
                            },
            ...
            add_action(
                ...
                function ($type_registry) {
                    ...
                    foreach ($this->get_editor_graphql_types() as $type) {
                        ...
                        register_graphql_field($type, 'preview_blocks', [
                            'type' => [
                                'list_of' => $this->get_graphql_block_interface_type()
                            ],
                            'description' => 'Gutenberg preview blocks',
                            'args' => [
                                'json' => Type::string()
                            ],
                            'resolve' => function ($post, $args) {
